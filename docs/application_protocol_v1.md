...
| Bit | 含义 | 第一版行为 |
|-----|------|-----------|
| bit0 | **请求 ACK** (1=需要, 0=不需要) | 接收方解码成功后检查,若=1 则自动回 0x00 ACK 帧(两条边界见 §功能码 0x00) |
...
- 广播帧(若未来支持 station_id=255)即使 bit0=1 也不回 ACK(避免总线冲突)。**第一版未实现此例外**:§站号策略 把广播推迟到第二版,协议层暂不定义 255,`maybe_reply_ack` 也就没有对应的守卫。启用广播时必须同时补上这个检查。
...
**第一版行为**: 协议节点**解码成功**且 `flags.bit0=1` 时,自动构造 0x00 帧回复到驱动 TX;发送方收到 0x00 时解码并日志 INFO `Decoded 0x00: acked_seq=?, acked_function_code=0x??`,但不等待、不超时重传(第二版实现)

**ACK 自身永远不请求 ACK。** 0x00 的 `data` 没有 flags 字节——偏移 0 处是 `acked_seq` 的低半字节。实现中 `flags_of()` 对 ACK 返回"无 flags"而非默认 0,两者的区别是实质性的:若把该字节当作 flags 读,任何**奇数** `acked_seq` 的 bit0 都是 1,会被误判为"请求 ACK";而回复本身又是一个 0x00 帧,两端协议节点将陷入无限互 ACK。

**自动回复的两条边界**(与 §帧去留清单配套,实现见 `ProtocolBridgeNode::maybe_reply_ack`):

1. **只回复解码成功的帧**。未知功能码的帧不回 ACK:协议层不知道它的 `data` 布局,偏移 0 处的字节未必是 flags,照它回复等于凭空捏造一个对方没发出的请求。此类帧按清单第 2 行仅转发。
2. **重复帧照回不误**。ACK 在重复判定**之前**发出。对端重发的最可能原因就是上一个 ACK 丢了;认出重复后保持沉默,会让它一直重传下去。「是否已收到」与「应用是否需要第二份」是两个问题:前者答 ACK,后者由清单第 6 行答丢弃。

回复帧的 `station_id` 填收到帧的 `station_id`(回给发送方)。**注意两侧机制不同**:主机侧驱动遵从 `msg->station_id`,回址真实生效;从机侧驱动会用启动参数 `station_id_` 覆盖此字段(RX 时填的是 0"来自主机",与 TX 覆盖值并不相同),回复能到达主机是因为驱动**丢弃**了我们写的值,而非两值相等。协议层照填不误,但不要据此以为该字段在从机侧有意义。
...
| # | 情况 | 处置 | 日志级别 | 诊断计数 |
|---|------|------|----------|----------|
| 1 | 解码成功(0x00 / 0x10 / 0x12) | **转发** | INFO(字段) | — |
| 2 | 未知或未实现功能码(0x11 / 0x80 / 其它) | **转发** | WARN | 未知功能码计数 |
...
本表只管"转发还是丢弃"。自动回复 ACK 是**另一条正交的出向动作**,不改变任何一行的处置:第 1 行和第 6 行的帧只要 `flags.bit0=1` 都会被回复(见 §功能码 0x00),第 2、3 行因未解码而不回复。
...
**行为**:
- RX 路径:decode data → 提取 flags/seq/payload → 若 flags.bit0=1 则自动回 0x00 → 检查序号/重复 → 非重复则发布到 `/protocol/rx`(顺序见 §功能码 0x00 的"两条边界")
